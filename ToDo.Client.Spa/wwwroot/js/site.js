...
var apiBaseAddress = "https://localhost:5001";
...
function getAllTodo() {
    clientApplication.acquireTokenSilent(request).then(tokenResponse => {
        fetchAllTodo(tokenResponse.accessToken);
    }).catch(function (error) {
        clientApplication.loginPopup(request).then(function (loginResponse) {
            getToken(request).then(tokenResponse => {
                fetchAllTodo(tokenResponse.accessToken);
            });
        }).catch(function (error) {
...
function deleteTodo(id, doneItem, accessToken) {
    $.ajax({
        url: `${apiBaseAddress}/api/todo/${id}`,
        headers: {
            'Authorization': 'Bearer ' + accessToken,
        },
        type: "DELETE",
...
function updateTodo(todo, elem, accessToken) {
    $.ajax({
        url: `${apiBaseAddress}/api/todo/${todo.id}`,
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + accessToken,
